    ...
    match ty {
        ...
        Ty::Int | Ty::F64 | Ty::F32 => {
            ...
            if roll < 4 {
                ...
                let lt = if matches!(ty, Ty::F64 | Ty::F32) { Ty::ListF64 } else { Ty::ListInt };
                let l = gen_expr(&lt, ctx);
                let b = if matches!(ty, Ty::F64 | Ty::F32) && matches!(lt, Ty::ListInt) {
        ...
        Ty::ListInt |         Ty::ListF64 | Ty::ListF32 => {
            ...
            ctx.depth_budget -= 1;
            let elem_f = matches!(ty, Ty::ListF64 | Ty::ListF32);
...
pub fn generate(g: &Gen, count: usize, seed: u64) -> Vec<Generated> {
    ...
    for i in 0..count {
        ...
        for j in 0..extra {
            let t = if matches!(g.ret, Ty::F64 | Ty::F32) && j == extra - 1 {
                g.ret.clone()
            } else {
                g.ret.clone()
            };
